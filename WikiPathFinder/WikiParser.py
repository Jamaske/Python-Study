...
class get_links:
    ...
    def __init__(self):
        try:
            with open(get_links.page_parse_cache, "rb") as file:
                ...
                print(f"page cache with: {len(self.storage)} entrys is successfully loaded")
        except Exception:
            self.storage = {}
    ...
    def __call__(self, url:str)->str:
        ...
        links = get_links.request_and_parse(url)
    ...
    def __del__(self):
        with open(get_links.page_parse_cache, "wb") as file:
            ...
            print(f'page cache with {len(self.storage)} entrys is successfully stored')
    

    @staticmethod
    def request_and_parse(url:str)->str:
...
if __name__ == "__main__":
    ...
    links = get_links.request_and_parse(math_URL)
